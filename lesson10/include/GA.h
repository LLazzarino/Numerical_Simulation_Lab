...
class GeneticLab{
  ...
  public:
    ...
    uvec Migrate_Leave(int);                                                            // Returns the indices of some of the best chromosomes, which will be leaving the country
    void Migrate_Welcome(rowvec, int);                                                  // Saves the received chromosome in a location left "empty"
    int GetNAlleles() {return _Nalleles;};                                              // Returns number of alleles
    void Inject_New(int);
    uvec Select_Worst(int);
