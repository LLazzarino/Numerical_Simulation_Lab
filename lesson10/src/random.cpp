...
Random :: Random(string Primefile, string seeds){
  int seed[4];
  int p1, p2;
  ifstream Primes(Primefile);
  if (Primes.is_open()){
     Primes >> p1 >> p2 ;
...
// Constructor that reads from two files
...
double Random :: UnifStep(double max){
   // This function generates a uniform step T(x|y) of width max to use in metropolis
   return -max+2*max*Rannyu();
}

double Random :: GaussStep(double sigma){
   // This function generates a gaussian step T(x|y) of width sigma to use in metropolis
   double s=Rannyu();
   double t=Rannyu();
...
double Random :: Exp(double lambda) {
   // This function generates a random number from a Gaussian distribution with given lambda
   double s=Rannyu();
