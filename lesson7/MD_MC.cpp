...
int main()
{
  ...
  //Thermalization
  for(int step=1; step <= thermalization_steps; step++)
  {
    Move();
  }
...
//Read input informations
  ...
  tailCorrection_v = 8 * pi * rho * (1./9./pow(rcut,9)-1./3./pow(rcut,3));
  tailCorrection_p = 32 * pi * rho * (1./9./pow(rcut,9)-1./6./pow(rcut,3));
  cout << "Tail correction for the potential energy = " << tailCorrection_v << endl;
  cout << "Tail correction for the virial = " << tailCorrection_p << endl;
  ...
  ReadInput >> thermalization_steps;

  cout << "The program perform Metropolis moves with uniform translations" << endl;
  cout << "Moves parameter = " << delta << endl;
  cout << "Number of blocks = " << nblk << endl;
  cout << "Number of steps in one block = " << nstep << endl;
  cout << "Steps done in order to thermalize the system (Molecular Dynamics) = " << thermalization_steps <<endl << endl;
  ReadInput.close();
...
//Prepare arrays for measurements
  ...
  ie = 3; //Total energy
  iw = 4; //Virial
  // then from 5 to 54 included there's g(r), with r from 0 to L/2
  n_props = 55; //Number of observables
  // useful for g(r)
  deltar = box / 100.;
...
//Print initial values for measured properties
  cout << "Initial potential energy = " << walker[iv]/(double)npart + tailCorrection_v << endl;
  ...
  cout << "Initial total pressure   = " << walker[iw] + tailCorrection_p * rho << endl;
...
void Move()
{
  ...
  if(iNVET) // Monte Carlo (NVT) move
  {
    ...
    //New
      x[o] = Pbc( x[o] + delta*(rnd.Rannyu() - 0.5) ); //transition kernel is uniform centered in zero
...
// distance i-j in pbc
  ...
  walker[iw] = rho*walker[it] + (1/(3.0*vol)) * 48.0 * w; // Pressure
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=14;
    ...
    stima_pot = blk_av[iv]/blk_norm/(double)npart + tailCorrection_v; //Potential energy
    ...
    stima_pres = blk_av[iw]/blk_norm + tailCorrection_p * rho; //Pressure ??? 3. * ???
    glob_av[iw] += stima_pres;
    glob_av2[iw] += stima_pres*stima_pres;
    err_pres=Error(glob_av[iw],glob_av2[iw],iblk);

    for(int i=5; i<n_props; i++){
      VolumeShell = 4. * pi / 3. * ( pow((i-4)*deltar,3) - pow((i-5)*deltar,3) );  //could be stored in origin
      stima_gofr = blk_av[i]/blk_norm/rho/(double)npart/VolumeShell /((double)npart * ((double)npart -1)/2); //g(r) and its normalization
...
void ConfFinal(void)
{
  ofstream WriteConf, WriteVelocity, WriteSeed, gofr;
  const int wd=14;
  ...
  gofr.open("output_gofr.dat");
  for(int i=0; i<50;i++){
    gofr << setw(wd) << i <<  setw(wd) << glob_av[i+5]/(double)nblk << setw(wd) << err_gofr[i+5] << endl;
  }
  gofr.close();
  rnd.SaveSeed();
}
...
double Pbc(double r)  //Algorithm for periodic boundary conditions with side L=box -> because of rint, it's -L/2 to L/2
{
