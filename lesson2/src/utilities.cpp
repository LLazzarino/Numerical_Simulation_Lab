...
vec blockAverageRMS(vec data, int Nblocks){
  //calculate the average and error of the RMS from a dataset by dividing it in blocks
  int Ndata=data.size();
  int blocksize = int(Ndata/Nblocks);

  vec sums(Nblocks, fill::zeros);
  vec RMSaverages(Nblocks, fill::zeros);
  vec averages2(Nblocks, fill::zeros);
  ...
  for(int i=0; i<Nblocks; i++){
    ...
    RMSaverages(i) = sqrt(sums(i) / blocksize);
  }

  //calculate the average and error using all blocks
  averages2 = RMSaverages % RMSaverages;
  double result = mean(RMSaverages);
  double error = sqrt( (mean(averages2) - result*result) / (Nblocks-1) );
